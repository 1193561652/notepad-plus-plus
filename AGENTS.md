...
- 重要插件首先尝试兼容官方未修改的原版 Windows DLL；只有真实源码、消息和行为
  证据证明有限兼容代价过高或违反架构边界时，才转入源码移植。
- 难以兼容原版 DLL 的重要插件，默认保持插件边界并重构为 Qt 版插件，复用原插件
  业务逻辑并替换宿主调用和平台 UI。除非该能力经单独决策属于编辑器核心职责，
  不得直接并入 Qt 主程序作为原生功能。
