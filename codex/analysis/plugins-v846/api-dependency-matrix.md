...
| 证据级别 | 数量 | 可作结论 | 禁止推断 |
| --- | ---: | --- | --- |
| 对应版本源码 | 62 | 对应 revision 实际业务调用、许可证、运行时和代理风险 | 未扫描路径、发布包运行行为、x86 实机稳定性 |
| 对应版本证据不足 | 107 | JSON 身份、版本、包哈希、主页和用户可见描述 | 具体 `NPPM_*`、`NPPN_*`、`SCI_*`、导出、依赖或兼容性 |
...
下表归并 62 项源码记录中确认的调用。单个插件的精确集合仍以其批次记录为准。
...
- 生命周期：`NPPN_READY`、`NPPN_BEFORESHUTDOWN`、`NPPN_SHUTDOWN`。
- 文档：`NPPN_BUFFERACTIVATED`、`NPPN_FILEOPENED`、`NPPN_FILEBEFORECLOSE`、
  `NPPN_FILECLOSED`、`NPPN_FILEBEFORESAVE`、`NPPN_FILESAVED`。
- UI 与配置：`NPPN_LANGCHANGED`、`NPPN_WORDSTYLESUPDATED`、
  `NPPN_DARKMODECHANGED`、`NPPN_TBMODIFICATION`、`NPPN_SHORTCUTREMAPPED`。
- 通信：`NPPN_CMDLINEPLUGINMSG`。
- Scintilla：`SCN_CHARADDED`、`SCN_MODIFIED`、`SCN_UPDATEUI`、`SCN_PAINTED`、
  `SCN_MARGINCLICK`、`SCN_ZOOM`、`SCN_FOCUSIN` 和折叠/保存点通知。

通知必须在 UI 线程发送，并保持主/副视图来源 HWND、Buffer ID、关闭前后顺序和
回调重入边界。高频 `SCN_MODIFIED/UPDATEUI` 不能每次构造全文副本。
...
| B | 23 | 需要有限新增消息、Dock、运行时或线程保护后试验 |
| C | 25 | direct call、复杂窗口结构、脚本宿主或特殊视图，优先源码移植/新 API |
| D | 1 | 价值不足以承担兼容风险，不支持 |
| U | 107 | 证据不足，不加载即不声称兼容 |
...
- B 类语料：`JsonTools`、`NPPJSONViewer`、`XMLTools`，用于缓冲区、Dock 和运行时。
