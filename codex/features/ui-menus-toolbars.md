...
- 此处是阶段六历史状态；Dark Mode 后续已覆盖图标、边框、Dock、菜单和常用控件。
...
- 已重新采集原版 v8.4.6 浅色/深色基线和 Qt 100%/150% 截图矩阵。
- 主窗口、Find 默认客户区和 Preferences 外框尺寸已经接近原版。
- 新文档窗口标题已恢复为 `new 1 - Notepad++`。
- Preferences 已恢复原版单一 Close/关闭命令。
- Preferences 内部页面、Shortcut Mapper 和 Project Panel 已在后续收口中按
  v8.4.6 页面职责、控件层级和工作流重建。
- Windows 100% DPI 已达到当前像素对照基线；平台字体和系统控件绘制不承诺逐字节一致。
