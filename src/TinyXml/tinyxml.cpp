...
void TiXmlElement::Print( std::string& outputStream, int depth ) const
{
	...
	//generic_fprintf( cfile, L"<%ls", value.c_str() );
...
void TiXmlAttribute::Print( std::string& outputStream, int /*depth*/ ) const
{
	...
	if (value.find('\"') == TIXML_STRING::npos)
	{
		...
		attrVsValue += "\"";
		//generic_fprintf(cfile, L"%ls=\"%ls\""), n.c_str(), v.c_str());
	}
	else
	{
		attrVsValue += "='";
...
{
	if ( generic_sscanf( value.c_str(), L"%d", ival ) == 1 )
...
{
	if ( generic_sscanf( value.c_str(), L"%lf", dval ) == 1 )
		return TIXML_SUCCESS;
	return TIXML_WRONG_TYPE;
}
...
void TiXmlText::Print( std::string& outputStream, int /*depth*/ ) const
{
	...
	//generic_fprintf( cfile, L"%ls", buffer.c_str() );
}
...
void TiXmlDeclaration::Print( std::string& outputStream, int /*depth*/ ) const
{
	...
	if (!version.empty())
	{
		...
		xmlDcl += "\" ";
		//generic_fprintf(cfile, L"version=\"%ls\" "), version.c_str());
	}
	...
	if (!encoding.empty())
	{
		...
		xmlDcl += "\" ";
		//generic_fprintf(cfile, L"encoding=\"%ls\" "), encoding.c_str());
	}
	if (!standalone.empty())
	{
		xmlDcl += "standalone=\"";
		...
		xmlDcl += "\" ";
		//generic_fprintf(cfile, L"standalone=\"%ls\" "), standalone.c_str());
	}
