...
{
public:
    explicit QsciDocumentP(int document_options = 0)
        : doc(0), nr_displays(0), nr_attaches(1), modified(false),
    ...
    int options;            // Options used when the document is first shown.
};
...
// Create a document that will use the requested Scintilla options when first
// displayed.
QsciDocument::QsciDocument(int options)
{
    pdoc = new QsciDocumentP(options);
}
...
void QsciDocument::display(QsciScintillaBase *qsb, const QsciDocument *from)
{
    ...
    bool release_created_document = false;
    if (!ndoc && pdoc->options != 0)
    {
        ndoc = qsb->SendScintillaPtrResult(
                QsciScintillaBase::SCI_CREATEDOCUMENT, 0, pdoc->options);
        release_created_document = (ndoc != 0);
    }
    ...
    if (release_created_document)
        qsb->SendScintilla(
                QsciScintillaBase::SCI_RELEASEDOCUMENT, 0, ndoc);
