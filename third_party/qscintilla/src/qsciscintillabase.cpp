...
// Send a message without narrowing pointer-width positions on Win64.
qintptr QsciScintillaBase::SendScintillaNpp(unsigned int msg,
        quintptr wParam, qintptr lParam) const
{
    return static_cast<qintptr>(sci->WndProc(
            msg, static_cast<uptr_t>(wParam), static_cast<sptr_t>(lParam)));
}


// Send a parameterized message that returns a pointer.
